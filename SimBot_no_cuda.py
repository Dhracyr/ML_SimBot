...
def reproduce(ga_env, mutation_rate, new_population, pop_size, population, action_space_n,
    ...
    return new_population, saved_damage_peak, generations_without_improvement, mutation_rate
...
def genetic_algorithm(ga_env, pop_size, generations, sequence_length, mutation_rate):
    ...
    for generation in range(generations):
        ...
        print(
            f"Generation {generation}: Max Damage {max_damage} of {GLOBAL_MAX_DAMAGE}, that's {max_damage / GLOBAL_MAX_DAMAGE * 100:.2f}%")
        ...
        if saved_damage_peak == 0:
            saved_damage_peak = max_damage
        if max_damage == saved_damage_peak:
...
global_tournament_k_amount = 8  # 10% of pop_size
